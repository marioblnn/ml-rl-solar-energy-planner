"""Predict And Plan Energy Generation — orchestration layer.

This module is the single coordination point between the weather API, the
scikit-learn solar model, the PPO scheduling agent, and Streamlit session
state. It never renders UI itself; it only computes state and calls
``st.error`` when an external dependency fails so the dashboard can keep
running with a safe fallback schedule.
"""
...
def _safe_session_get(key: str, default: Any = None) -> Any:
    """Read session state defensively so a missing key never crashes a rerun."""
    try:
...
def getLocationTimezone(location: Optional[str] = None) -> ZoneInfo:
    """Return the IANA timezone for the selected (or given) location."""
    if location is None:
...
def getLocationNow(location: Optional[str] = None) -> datetime:
    """Return the current date/time in the selected location's timezone."""
    return datetime.now(getLocationTimezone(location))


def getLocationCurrentHour(location: Optional[str] = None) -> int:
    """Return the current hour (0–23) in the selected location's timezone."""
    return getLocationNow(location).hour


def ensureSessionDefaults() -> None:
    """Create any planner-related keys this module relies on, if absent."""
    try:
        defaults = {
            KEY_DEVICES: [],
...
def fetchWeatherForecast(location: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """Fetch a 24-hour hourly forecast from Open-Meteo for the location.

    Returns a dict of parallel hourly arrays, or ``None`` on any failure so the
    caller can fall back to a rule-based forecast without crashing.
    """
    if location is None:
        location = _safe_session_get(KEY_SELECTED_LOCATION, "Bucharest")
...
def loadSolarModel() -> Optional[Any]:
    """Load solar_model.joblib (cached), or return None and surface an error."""
...
def generateSolarForecast(
    ...
    """Produce 24 hourly solar estimates (kWh) from weather features.

    Falls back to a deterministic daytime bell curve when the model or weather
    data is unavailable, guaranteeing the dashboard always has a forecast.
    """
...
def _series_value(weather: Dict[str, Any], key: str, hour: int) -> float:
    """Safely read one hour from a weather series, defaulting to 0.0."""
    try:
...
def _fallback_solar_curve() -> List[float]:
    """Deterministic bell curve peaking near noon; safe offline default."""
    import math
...
def loadPPOAgent() -> Optional[Any]:
    """Load the trained PPO agent (cached), or return None on any failure."""
...
class _PlanContext:
    """Everything the optimizer needs, extracted once from session state.

    The home is modelled hour by hour from ``start`` (the current local hour) to
    23:00. Three kinds of load exist:

    * **background** — devices that are manually ON with the Smart Optimizer
      OFF. The agent cannot touch these; it must plan *around* their fixed draw.
    * **forced** — smart devices inside a priority window for a given hour, which
      are non-negotiably ON.
    * **flexible** — the remaining required runtime of smart devices, which the
      optimizer is free to place in the cheapest hours.
    """
...
def _simulate(on_hours: Dict[str, List[int]], ctx: _PlanContext) -> Dict[str, Any]:
    """Replay a candidate schedule hour by hour and tally grid usage.

    Solar first covers the hour's load; any surplus charges the battery, any
    deficit is covered by the battery before the remainder is bought from the
    grid. This is the optimal battery policy under a flat tariff, so the only
    thing that differs between candidate plans is *when* devices run.
    """
...
def _ranked_hours(ctx: _PlanContext) -> List[int]:
    """Plannable hours ordered by solar (sunniest first), earliest breaks ties.

    Preferring earlier hours among equals lets the battery bank surplus that can
    later cover an evening deficit, which is exactly the "memory" role of the
    accumulator.
    """
...
def _greedy_decisions(ctx: _PlanContext) -> Dict[str, List[int]]:
    """Deterministic planner: force priority hours, then fill runtime into sun.

    Big loads pick their hours first so the heaviest consumers grab the sunniest
    slots; the battery simulation then mops up the rest. This is the always-good
    fallback used whenever the RL agent is unavailable.
    """
...
def _rl_decisions(
    ctx: _PlanContext, agent: Any, devices: List[Device]
) -> Optional[Dict[str, List[int]]]:
    """Roll out the trained PPO policy over the current day, then repair it.

    The policy chooses which smart devices run each hour; we map its decisions
    back onto the named devices and *repair* the result so hard constraints
    always hold: priority hours are forced ON, and any device short of its
    required runtime is topped up in the sunniest remaining hours. This keeps
    the agent in charge of optimization while guaranteeing a feasible plan.
    """
    try:
...
def buildPlannerSchedule(
    ...
    """Produce the planner table plus the plan data needed by the dashboard.

    Returns ``(schedule_blocks, plan_data)`` where ``plan_data`` carries the
    per-device ON hours, the whole-day grid usage/cost and the chosen source.

    Strategy: always compute the deterministic greedy plan; when a trained agent
    is available, also roll it out and keep whichever schedule draws less grid
    energy. The RL agent therefore drives the plan whenever it is at least as
    good as the heuristic, while the heuristic guarantees the table is never
    worse than a sensible baseline and is always present.
    """
...
def _build_blocks(
    ...
    """Turn hourly decisions into compact planner rows (devices + battery)."""
...
def _battery_blocks(per_hour: List[float], label: str) -> List[Dict[str, Any]]:
    """Merge consecutive non-zero battery-flow hours into labelled blocks."""
...
def _solar_at(forecast: List[float], hour: int) -> float:
    """Safely read one hour of the solar forecast, defaulting to 0.0."""
    try:
...
def _merge_hour_set(hours: set) -> List[Tuple[int, int]]:
    """Collapse a set of integer hours into (start, end_exclusive) blocks."""
...
def detectStateChanges() -> bool:
    """Return True if any replan-relevant control changed since the last plan."""
    try:
...
def needsReplan() -> bool:
    """A replan is needed if explicitly flagged OR relevant state changed."""
    flagged = bool(_safe_session_get(KEY_NEEDS_REPLAN, False))
    return flagged or detectStateChanges()


def _compute_state_hash() -> str:
    """Hash the inputs that must trigger a full replan when they change.

    Crucially, a *smart* device's manual ``is_on`` is excluded: orchestrate sets
    it from the plan for the current hour, and including it would create an
    endless replan loop. The current hour IS included so the schedule advances
    with the day. Only a non-smart device's manual state matters (background).
    """
...
def updateScheduleInSessionState(
    ...
    """Persist the schedule + plan data, refresh metrics, clear replan flags."""
    try:
...
def _apply_current_hour_state(device_hours: Dict[str, List[int]]) -> None:
    """Switch smart devices ON/OFF to match the plan for the current hour.

    If the plan runs a device now, it is turned on so its draw shows up in the
    live "Consuming" / grid metrics; otherwise it is turned off. Non-smart
    devices are left exactly as the user set them.
    """
    try:
        hour = getLocationCurrentHour()
        devices = _safe_session_get(KEY_DEVICES, [])
...
def orchestrate(force: bool = False) -> List[Dict[str, Any]]:
    """Main coordination routine called by dashboard.py on each rerun.

    Runs the full predict-and-plan pipeline only when a replan is required,
    otherwise returns the cached schedule to keep reruns cheap.
    """
    ...
    # 1) Weather: pull a fresh forecast for the selected location.
    location = _safe_session_get(KEY_SELECTED_LOCATION, "Bucharest")
    ...
    # 2) Solar: load model and predict 24-hour generation.
    ...
    # 3) RL: load agent and build the daily schedule (smart devices only).
    agent = loadPPOAgent()
    devices = _safe_session_get(KEY_DEVICES, [])
    ...
    # 4) Persist results and reset the replan trigger.
...
def _priority_hours_for(device: Device) -> List[int]:
    """Convert a device priority window into a list of integer hours."""
    try:
...
def _accumulator_level() -> float:
    """Read the current battery charge from the session Accumulator/float."""
    try:
...
def _report_error(message: str) -> None:
    """Surface an error in the UI without raising; safe outside Streamlit too."""
    try:
        st.error(message)
    except Exception:
        pass
