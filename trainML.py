"""Train a Random Forest model to predict hourly solar generation from weather data."""
...
def loadAndPreprocessData(csv_path: str | Path = DEFAULT_CSV_PATH) -> Tuple[pd.DataFrame, pd.Series]:
    """Load solar_data.csv and return feature matrix X and target vector y."""
    ...
    invalid_ts = df["hour_ts"].isna()
    if invalid_ts.any():
        dropped = int(invalid_ts.sum())
        print(f"Dropping {dropped} row(s) with unparseable timestamps.")
...
def trainRandomForest(
    ...
    *,
    ...
    """Train a RandomForestRegressor using an 80/20 train/test split."""
...
def evaluateAndSaveModel(
    model: RandomForestRegressor,
    X_test: pd.DataFrame,
    y_test: pd.Series,
    model_path: str | Path = DEFAULT_MODEL_PATH,
) -> Tuple[float, float]:
    """Evaluate on the held-out test set, print metrics, and persist the model."""
    predictions = model.predict(X_test)
    mae = float(mean_absolute_error(y_test, predictions))
    r2 = float(r2_score(y_test, predictions))

    print("--- Solar Generation Model Evaluation ---")
...
def main() -> int:
    """Run the full training pipeline when executed as a script."""
