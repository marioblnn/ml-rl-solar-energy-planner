...
class SmartHomeEnv(gym.Env):
    ...
    def step(self, action: Sequence[int]) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
    ...
    #Energy balance
    def _apply_energy_balance(self, solar: float, load: float) -> Tuple[float, float, float]:
    ...
    #Background devices consumption
    ...
    #Sistemul de normalizare a starii
    #Ia features si le transforma intr-un vector de observari normalizat in [0, 1] 
...
def make_training_env(seed: Optional[int] = None) -> SmartHomeEnv:
    ...
    return SmartHomeEnv(consumption=consumption,solar_forecast=_default_solar_forecast(),randomize=True,seed=seed)


def trainAndSavePPO(env: Optional[SmartHomeEnv] = None, total_timesteps: int = TRAIN_TIMESTEPS, model_path: Path = MODEL_PATH) -> str:
